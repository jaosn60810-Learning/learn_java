...
public class GetProfessionCard {
    ...
    public static Card[] setWarrior() {
        Card[] tmp = new Card[]{
                new Card(1, "照顧新人的戰士\t", 1, "自己之後打出的卡片數值 * 2", 5, 2, 1, 1, 1) {
                    ...
                    @Override
                    protected void actionForAllTheTime(Player p1, Player p2) {
                ...
                new Card(5, "多拿一把劍的戰士\t", 2, "抽一張牌", 5, 2, 1, 3, 1) {
                    ...
                    @Override
                    protected void actionForOneTime(Player p1, Player p2) {
                        Card card = p1.getCardonDesk()[p1.getCardonDesk().length - 1];
                        if (card.getEffectCount() > 0) {
    ...
    public static Card getProfessionOneCard(int cardNum) {
        Card[] tmpAllCards = new Card[40];
        Card[] jobCards = new Card[10];
        // 戰士
        jobCards = setWarrior();
        for (int i = 0; i < 10; i++) {
            tmpAllCards[i] = jobCards[i];
        }
        // 牧師
        jobCards = setPriest();
        for (int i = 0; i < 10; i++) {
            tmpAllCards[i + 10] = jobCards[i];
        }
        // 法師
        jobCards = setWizard();
        for (int i = 0; i < 10; i++) {
            tmpAllCards[i + 20] = jobCards[i];
        }
        // 武僧
        jobCards = setMonk();
        for (int i = 0; i < 10; i++) {
            tmpAllCards[i + 30] = jobCards[i];
        }

        for (Card c : tmpAllCards) {
            if (c.getNumber() == cardNum) {
                return c.getThisCard();
            }
        }
        return null;
    }
