...
public class Desk {
    ...
    public void Fight(int priority, int AI, int mode, int rounds) throws InterruptedException {
        chooseCardSet = input("請選擇牌組:", 1, player1.getCardSetCount(), false);
        /**
         * 打亂牌組順序
         */
        randomCardSet(player1.getTotalCardSets()[chooseCardSet -1]);
        randomCardSet(computer.getTotalCardSets()[chooseCardSet -1]);
    ...
    private void DrawCards(int drawNum, Player player) {
        ...
        //抽五張牌
        for (int j = drawNum -1; j >= 0; j--) {
            temp[j] = player.getTotalCardSets()[chooseCardSet-1].getCards()[j];
            System.out.println("測試");
            temp[j].printOutInfo();
            player.getTotalCardSets()[chooseCardSet-1].removeCard(j);
        }
    ...
    private void computerAction( int AI, int mode) {
        //如果是快速戰鬥,出牌前丟棄
    ...
    private void playerAction(int mode) {
        //如果是快速戰鬥,出牌前丟棄
    ...
    private void actionStart(int AI, int mode, int priority){
        /**預設一開始雙方的出牌許可都是true*/
        p1CanPlay = true;
        p2CanPlay = true;
        do {
     ...
     * 電腦出牌邏輯
     */
    public void computerPlay(int AI){
        //聰明模式
        if (AI == 1) {
            int sump1 = 0, sump2 = 0;
    ...
    private void randomCardSet(CardSet cardSet) {
        ...
        for (int i = 0; i < shuffleCard.length; i++) {
            ...
            shuffleCard[i] = cardSet.getCards()[i];
