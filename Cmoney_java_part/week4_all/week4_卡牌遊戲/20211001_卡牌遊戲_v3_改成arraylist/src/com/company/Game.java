...
public class Game {
    ...
    public void gameStart() throws InterruptedException {
        ...
        //將玩家1的牌組設定屬於玩家
        for (int i = 0; i < player1.getOneOfCardset(chooseCardSet).size(); i++) {
            player1.getOneOfCardset(chooseCardSet).get(i).setOwner(0);
        }
        //將電腦的牌組設定屬於電腦
        for (int i = 0; i < player2.getOneOfCardset(1).size(); i++) {
            player2.getOneOfCardset(1).get(i).setOwner(1);
        }
