...
public class Desk {
        ...
        if (mode == 2) {
            if (player.getCardonDesk().length == 3) {
                if (player == player1) {
                    ...
                    //先將牌桌上的牌加到棄牌區
                    player.setCardOut(addToCardSet(player.getCardOut(), player.getCardonDesk()[input - 1]));
                    //再將牌桌上的牌移除掉
                    ...
                    //先將牌桌上的牌加到棄牌區
                    player.setCardOut(addToCardSet(player.getCardOut(), player.getCardonDesk()[input]));
                    //再將牌桌上的牌移除掉
    ...
    private void cleanDesk() {
        for (int j = player1.getCardonDesk().length - 1; j >= 0; j--) {
            //先將牌桌上的牌加到棄牌區
            player1.setCardOut(addToCardSet(player1.getCardOut(), player1.getCardonDesk()[j]));
            //再將牌桌上的牌移除掉
            player1.setCardonDesk(removeCardFromCardSet(player1.getCardonDesk(), j));
        }
        for (int j = computer.getCardonDesk().length - 1; j >= 0; j--) {
            computer.setCardOut(addToCardSet(computer.getCardOut(), computer.getCardonDesk()[j]));
    ...
    private void getFiveCards() {
        ...
        //抽五張牌
        for (int j = 4; j >= 0; j--) {
            player1.setHandCard(addToCardSet(player1.getHandCard(), Card.copy(player1.getCardSet()[j])));
            player1.setCardSet(removeCardFromCardSet(player1.getCardSet(), j));
        }
        //電腦抽五張牌
        System.out.println("===================================");
        for (int j = 4; j >= 0; j--) {
            computer.setHandCard(addToCardSet(computer.getHandCard(), Card.copy(computer.getCardSet()[j])));
    ...
    private void checkOnDesk(Player player) {
        ...
        for (int i = checkPoint; i >= 0; i--) {
            ...
            if (tmp.getPoint() <= 0) {
                ...
                player.setCardOut(addToCardSet(player.getCardOut(), player.getCardonDesk()[i]));
    ...
    public void computerPlay(int AI) throws InterruptedException {
        ...
        if (AI == 1) {
            int sump1 = 0, sump2 = 0;
            for (int i = 0; i < player1.getCardonDesk().length; i++) {
                sump1 += player1.getCardonDesk()[i].getPoint();
            }
            for (int i = 0; i < computer.getCardonDesk().length; i++) {
     ...
     * 打亂牌組順序
     */
    private void randomCardSet(Card[] cardSet) {
        Card tmp;
        for (int i = 0; i < cardSet.length; i++) {
            int randomIndex = random(0, cardSet.length - 1);
            tmp = Card.copy(cardSet[i]);
            cardSet[i] = Card.copy(cardSet[randomIndex]);
            cardSet[randomIndex] = tmp;
        }
    }
