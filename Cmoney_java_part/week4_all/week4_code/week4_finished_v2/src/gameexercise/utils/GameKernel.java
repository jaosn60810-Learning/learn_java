...
public class GameKernel extends Canvas {  
    ...
    public static class Builder {
        ...
        //開啟鍵盤、滑鼠的監聽系統，目前版本無開啟無法建立GameKernel
        ...
        */
        ...
        //因為滑鼠感測問題，有時候在滑鼠按下時，因為長短有時跑出pressed 有時是 click 開啟後可以強制都跑出pressed
    ...
    // 這裡的GI 也就是我們在main時創立的GI <- 而此GI就是我們已經自定義好裡面的內容了，包含場景、物件等等
...
//        g.setColor(Color.BLACK);
        ...
        if (gi != null) {
            gi.paint(g);  // 在此時會幫我們做GI paint 的動作
        }
        g.dispose();
        bs.show();
    ...
    public void run(boolean isDebug) {
        cs.start();  // start() 是CommandSolver 的thread 的功能 <- 不知為何而用
