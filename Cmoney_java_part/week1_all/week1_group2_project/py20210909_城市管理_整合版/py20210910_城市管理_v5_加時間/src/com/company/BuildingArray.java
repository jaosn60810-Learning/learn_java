...
public class BuildingArray {
    ...
    public void buildBarracks(int hour, Resource resource) {
        // 判斷是否已經有此建築
        for (Building build : buildings) {
            if (build instanceof Barracks) {
                System.out.println("已經有此建築");
                return;
            }
        }
        // 判斷資源是否足夠
        if (resource.getWood() >= 20 && resource.getSteel() >= 10) {
            buildings.add(new Barracks(hour));
            resource.setWood(resource.getWood() - 20);
...
//            int costWood = 15;
    ...
    public void buildArsenal(int hour, Resource resource) {
        ...
        if (resource.getWood() >= 30 && resource.getSteel() >= 10) {
            buildings.add(new Arsenal(hour));
    ...
    public void buildingsGenerateSoldiers(ArrayList<Building> buildings, int realWorldTime, Soldier soldiers) {
        for (Building build : buildings) {
            if (build instanceof Barracks) {
                ((Barracks) build).generateSoldier(soldiers, realWorldTime);
            }
        }
    }
